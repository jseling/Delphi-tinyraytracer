...
type
  TVector3f = record
    ...
    constructor Create(const _AX, _AY, _AZ: Single);
  ...
  function GetColor(_AColor: TVector3f): TColor; overload;
...
function GetColor(_AColor: TVector3f): TColor;
var
  AR,
  AG,
  AB: Byte;
begin
  if (_AColor.X < 0) or (_AColor.X > 1) or
     (_AColor.Y < 0) or (_AColor.Y > 1) or
     (_AColor.Z < 0) or (_AColor.Z > 1) then
     raise Exception.Create('Invalid parameter.');

  AR := Trunc(_AColor.X * 255);
  AG := Trunc(_AColor.Y * 255);
  AB := Trunc(_AColor.Z * 255);
  Result := RGB(AR, AG, AB);
end;
{ TSphere }
...
constructor TVector3f.Create(const _AX, _AY, _AZ: Single);
begin
  X := _AX;
  Y := _AY;
  Z := _AZ;
